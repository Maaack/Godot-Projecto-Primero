...
onready var centered_node = $MarginContainer/NinePatchRect/MarginContainer/TextureRect/CenterContainer/CenteredOverlay
...
func show_ship_mounts(ship_node:Node2D, scale_ratio:float):
	for mount in ship_node.get_children():
		if mount.is_in_group('MOUNT'):
			...
			print("mount.position", str(mount.position))
